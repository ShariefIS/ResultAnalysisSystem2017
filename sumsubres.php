...
onclick="window.print();return false;" /></form>        
  ...
  <tr>
      <th align="center" colspan="8">OU Subject Wise Analysis March-2017 Results</th>
  </tr>
  <tr>
      <th>Subject : </th>
  ...
  <tr>
   ...
   <?php
        $cn= mysqli_connect("localhost","root","","ras2017");
        if (!$cn) {
        die("Connection failed: " . mysqli_connect_error());
        }
    ...
    <?php endwhile;?>   
      ...
      <?php
        ...
        $sp="SELECT round((count(sresult)/count)*100,2) as res
                FROM (select count(rno)as count from student_list where year=$v1 and sem=$v2 and section in ('$v3'))as top , marks_info_it
                WHERE rno in (select rno from student_list where section in ('$v3') and year=$v1)
		and sresult in ('PASS')
                GROUP BY scode
                HAVING scode=$v4 "; 
