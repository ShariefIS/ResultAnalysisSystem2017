...
<table align="center" border="3">
  <tr>
      <th align="center" colspan="7">OU Class Topper Result March 2017</th>
  </tr>

  
  <tr>
      <th>Class :</th>
      <td align="center" colspan="2">B . E  ( <?php echo htmlspecialchars($_POST["year"]);?> / 4 )  I T D </td>
    <th>Section: </th>
  ...
  <tr>
   ...
   <?php
        ...
        $sql="SELECT section, rno, name, sum(total) AS ototal , round((sum(total)/775)*100,2) as per
            FROM (SELECT marks_info_it.rno, section, name ,IFNULL(emarks,0)+imarks AS total FROM marks_info_it,student_list WHERE scode in (select scode from subjects where year=$v1 and sem=$v2) and marks_info_it.rno=student_list.rno)  AS tb
            WHERE rno in  (select rno from student_list where section in ('$v3') and year=$v1)
            GROUP BY rno
            ORDER BY sum(total) DESC LIMIT $v4"; 
  ...
  <?php while ($row = mysqli_fetch_assoc($result)): ?>
    ...
    <td align=\"center\">{$row["ototal"]}</td>
    <td align=\"center\">{$row["per"]} % </td>             
    </tr>";
    ?>
    <?php $i++ ?>
    <?php endwhile;?>
